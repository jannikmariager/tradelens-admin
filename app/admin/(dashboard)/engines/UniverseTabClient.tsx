...
export function UniverseTabClient({ engineVersion, horizon, stats, performanceUniverse, researchUniverse }: Props) {
  ...
  const inPerformance = stats.filter((s) => performanceSet.has(s.ticker.toUpperCase()))
  const inResearch = stats.filter((s) => researchSet.has(s.ticker.toUpperCase()))
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-foreground">All {horizonConfig.label} Performance Tickers ({inPerformance.length})</h3>
          <div className="text-xs text-muted-foreground">
            Approved for {horizonConfig.label} trading
          </div>
        </div>
      ...
      <Card className="p-4 space-y-3">
        <h3 className="font-semibold text-foreground">Red Flag Tickers (in {horizonConfig.label})</h3>
        {thresholdBadge}
        {redFlags.length === 0 ? (
          <div className="text-sm text-muted-foreground mt-2">
            No red flags. All {horizonConfig.label} performance tickers are healthy.
          </div>
        ) : (
          <UniverseTable rows={redFlags} actionLabel="Demote" onAction={handleDemote} />
        )}
      </Card>
