...
async function VersionDetail({ version }: { version: string }) {
  ...
  if (!v) {
    return <div className="text-sm text-slate-400">Version {version} not found.</div>;
  }

  return (
    <div className="space-y-4">
      <h1 className="text-3xl font-bold text-white">Engine {version}</h1>
...
export default function EngineVersionPage({ params }: PageProps) {
  ...
  return (
    <QueryProvider>
      <Suspense fallback={<div className="text-sm text-slate-400">Loading version {version}…</div>}>
