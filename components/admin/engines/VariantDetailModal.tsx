...
export function VariantDetailModal({ open, onOpenChange, variantKey, aggregate, runs }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl rounded-2xl bg-slate-900 border-slate-800">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-slate-50">
            <span>{variantKey}</span>
            {aggregate?.engine_version && (
              <Badge variant="outline" className="rounded-full text-[11px] border-slate-600 text-slate-200">
                {aggregate.engine_version}
              </Badge>
            )}
          </DialogTitle>
        ...
        <div className="mt-4 max-h-[480px] overflow-y-auto space-y-6 pr-2">
          ...
          <div className="grid gap-4 md:grid-cols-4 text-xs">
            <div className="rounded-2xl bg-emerald-500/10 p-3">
              <div className="flex items-center gap-1 text-[11px] font-medium text-emerald-200">
            ...
            <div className="rounded-2xl bg-red-500/10 p-3">
              <div className="flex items-center gap-1 text-[11px] font-medium text-red-200">
            ...
            <div className="rounded-2xl bg-yellow-500/10 p-3">
              <div className="flex items-center gap-1 text-[11px] font-medium text-yellow-100">
                Stability score
              </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 text-xs">
            ...
            <div className="rounded-2xl border border-red-500/30 bg-red-500/5 p-3">
              <div className="mb-1 flex items-center justify-between text-[11px] font-semibold text-red-200">
          ...
          <div className="space-y-2 text-xs">
            <div className="flex items-center justify-between">
              <span className="font-medium text-slate-100">All runs ({variantRuns.length})</span>
                ...
                <tbody>
                  {variantRuns.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-1 text-right">{(r.sharpe ?? 0).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
